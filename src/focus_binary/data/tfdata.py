...
try:  # Optional import so the scaffolding works without TF installed yet.
    import tensorflow as tf
except Exception:  # pragma: no cover - tensorflow may not be present in the stub stage.
    tf = None

try:  # Optional, used for TIFF decoding.
    import tensorflow_io as tfio
except Exception:  # pragma: no cover - tensorflow-io may not be present.
    tfio = None
...
def _load_image(
    ...
    if image_mode == "grayscale" and force_rgb:
...
def build_datasets(
    ...
    """Build a tf.data.Dataset pipeline for a manifest and split."""
    _require_tf()
    ...
    def _map_fn(path, label):
        img = _load_image(path, input_size=input_size, image_mode=image_mode, force_rgb=force_rgb)
